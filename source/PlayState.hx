...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		// upscale characters
		if (dad.curCharacter != 'senpai' && dad.curCharacter != 'senpai-angry' && dad.curCharacter != 'spirit')
			dad.scale.set(2, 2);
		...
		_pad = new FlxVirtualPad(NONE, PAUSEB);
		_pad.alpha = 0.75;
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			switch (noteTypeCheck)
			{
				...
				case 'pixel':
					if (FlxG.save.data.noteskin == 1) //circle note
	...
	override public function update(elapsed:Float)
	{
		...
		if ((_pad.buttonPause.justPressed || FlxG.keys.anyJustPressed([pauseBind])) && startedCountdown && canPause && !cannotDie)
		{
