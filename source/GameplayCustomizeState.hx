...
class GameplayCustomizeState extends MusicBeatState
{
	...
	public override function create()
	{
		...
		text = new FlxText(5, FlxG.height + 40, 0,
			"Click and drag around gameplay elements to customize their positions. Press C to reset. UP/DOWN to change zoom. B to show combo. A to exit.",
			12);
		#else
		text = new FlxText(5, FlxG.height + 40, 0,
		...
		_pad = new FlxVirtualPad(NONE, A_B_C_UP_DOWN);
		_pad.alpha = 0.75;
	...
	override function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.Q || _pad.buttonUp.justPressed)
		{
			FlxG.save.data.zoom += 0.02;
			camHUD.zoom = FlxG.save.data.zoom;
		}

		if (FlxG.keys.justPressed.E || _pad.buttonDown.justPressed)
		{
			FlxG.save.data.zoom -= 0.02;
			camHUD.zoom = FlxG.save.data.zoom;
		}
		...
		if (FlxG.keys.justPressed.C || _pad.buttonB.justPressed)
		{
		...
		if (FlxG.keys.justPressed.R || _pad.buttonC.justPressed)
		{
			sick.x = defaultX;
			sick.y = defaultY;
		...
		if (controls.BACK || _pad.buttonA.justPressed)
		{
			#if desktop
			FlxG.mouse.visible = true;
			#else
